...
::: {.callout-tip appearance="simple"}
**What is a Q-Q plot?** This is likely the first one you've met, so take
it slowly. It compares your actual residuals to what you'd expect if
they were perfectly normally distributed. Each dot is **one
...
::: {.callout-tip appearance="simple"}
**Residual plots:** Jamovi gives you three. The **first (Fitted)** is
the important one, residuals plotted against your regression line's
predictions. The other two plot residuals against each individual
variable. In all of them you're looking for one thing: **funnelling**
(spread that grows or shrinks along the x-axis). An even band =
homoscedasticity ✓. A funnel = heteroscedasticity ✗, meaning the model
predicts some people much better than others, so predictions (and
p-values) become less reliable at the wide end of the funnel. Bonus: a
*curve* in this plot is also a red flag, it means the linearity
assumption is shaky. (You'll read your own residual plot in Task 4b.)
:::
...
::: {.callout-important appearance="simple"}
Jamovi's regression also reports **R** (capital, no square) under Model
Fit. It's always shown as *positive*, so it tells you strength but
**not** direction, that's why we ran the correlation first in Task 2
(which keeps the minus sign). To avoid confusion you can untick "R"
under `Model Fit` and take direction from your Task 2 correlation.
:::
...
the *only* point that meets the boxplot/3s rule.
...
to **report you tested both** and say what you landed on in the end.
