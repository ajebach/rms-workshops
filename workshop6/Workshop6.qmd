...
+=====================+================+=========================+
| *Do three or more   | **One-way      | The means of *k*        |
| separate groups     | ANOVA**        | independent groups      |
| differ?*            |                | (between-subjects)      |
+---------------------+----------------+-------------------------+
| *Do the **same      | **Repeated     | The means of *k*        |
| people** differ     | measures (RM)  | conditions measured in  |
| across three or     | ANOVA**        | the same participants   |
| more conditions or  |                | (within-subjects)       |
| times?*             |                |                         |
+---------------------+----------------+-------------------------+
| *Do two factors,    | **Two-way      | Two main effects and    |
| and their           | ANOVA**        | their interaction       |
| combination, affect |                | (concept only in this   |
| the outcome?*       |                | course)                 |
+---------------------+----------------+-------------------------+
...
+===================================+============================+
| "three groups", "randomised to    | One-way ANOVA              |
| one of four diets", "athletes,    |                            |
| students and controls"            |                            |
+-----------------------------------+----------------------------+
| "each participant tried all       | Repeated measures ANOVA    |
| three", "cross-over", "baseline,  |                            |
| 4 weeks and 8 weeks"              |                            |
+-----------------------------------+----------------------------+
| "does the effect of A depend on   | Two-way ANOVA              |
| B", two categorical predictors    | (interaction)              |
+-----------------------------------+----------------------------+
| only two groups/conditions        | *t*-test (an ANOVA on two  |
|                                   | groups gives the identical |
|                                   | answer: F = t²)            |
+-----------------------------------+----------------------------+
...
| **Assumptions**  | outcome,         | Descriptives and |
|                  | independent      | the ANOVA        |
|                  | people,          | dialog's         |
|                  | approximately    | *Assumption      |
|                  | normal in each   | Checks*          |
|                  | group, similar   |                  |
|                  | spread in each   |                  |
|                  | group (RM: plus  |                  |
|                  | sphericity)      |                  |
+------------------+------------------+------------------+
| 2\.              | H₀: μ₁ = μ₂ = μ₃ | Written *before* |
| **Hypotheses**   | (all group means | you analyse      |
|                  | equal).          |                  |
|                  |                  |                  |
|                  | Hₐ: at least one |                  |
|                  | mean differs     |                  |
+------------------+------------------+------------------+
| 3\. **Test       | F = MS~between~  | Jamovi ANOVA     |
| statistic**      | ÷ MS~within~,    | table, `F`       |
|                  | i.e. how far     |                  |
|                  | apart the group  |                  |
|                  | means are,       |                  |
|                  | relative to how  |                  |
...
|                  | groups           |                  |
+------------------+------------------+------------------+
| 4a. **p-value**  | Probability of   | Jamovi ANOVA     |
|                  | an F at least    | table, `p`       |
|                  | this large *if   |                  |
|                  | H₀ were true*,   |                  |
|                  | from the         |                  |
|                  | F-distribution   |                  |
|                  | with df₁ and df₂ |                  |
+------------------+------------------+------------------+
| 4b. **Post-hoc** | *Only if p \<    | Jamovi *Post Hoc |
| (if significant) | 0.05*: which     | Tests* table     |
|                  | pairs differ?    |                  |
|                  | Mean difference, |                  |
|                  | CI, Cohen's d,   |                  |
|                  | MCID for each    |                  |
|                  | pair             |                  |
+------------------+------------------+------------------+
| 5\.              | Reject / fail to | Your write-up    |
| **Conclusion**   | reject H₀, then  |                  |
|                  | interpret with   |                  |
|                  | η² and context   |                  |
+------------------+------------------+------------------+
...
+================+==================+===========================+
| -   \*         | Shapiro--Wilk p  | Descriptives with *Split  |
|                | \> 0.05,         | by*, or ANOVA ▸           |
| Normality\*\*  | bell-shaped      | Assumption Checks ▸       |
| (each group /  | histogram, Q-Q   | Normality test + Q-Q (one |
|                | dots on the      | test on the *residuals*   |
| condition)     | diagonal         | from all groups)          |
+----------------+------------------+---------------------------+
| -              | Levene's p \>    | ANOVA ▸ Assumption Checks |
|  \*Homogeneity | 0.05. If         | ▸ Homogeneity test        |
|     of         | violated, tick   |                           |
|                | *Don't assume    |                           |
|                | equal (Welch's)* |                           |
|   variance\*\* | correction and   |                           |
|     (one-way)  | report that row  |                           |
+----------------+------------------+---------------------------+
| -   \*         | Mauchly's p \>   | RM ANOVA ▸ Assumption     |
|                | 0.05. If         | Checks ▸ Sphericity tests |
| Sphericity\*\* | violated, tick   |                           |
|     (RM only)  | the G r          |                           |
|                | e                |                           |
|                | enhouse--Geisser |                           |
|                | correction and   |                           |
...
+===============+==================================================+
| **Sum of      | The raw variability. Factor row = how far the    |
| Squares       | group means sit from the grand mean (between).   |
| (SS)**        | Residuals row = how far individuals sit from     |
|               | their own group mean (within)                    |
+---------------+--------------------------------------------------+
| **df**        | k − 1 for the factor; N − k for the residuals    |
|               | (RM: (n − 1)(k − 1))                             |
+---------------+--------------------------------------------------+
| **Mean Square | SS ÷ df, i.e. the variability *per degree of     |
| (MS)**        | freedom*. This is what gets compared             |
+---------------+--------------------------------------------------+
| **F**         | MS~between~ ÷ MS~within~. Bigger the value =     |
|               | stronger evidence against H₀                     |
+---------------+--------------------------------------------------+
| **p**         | Right-tail probability of an F this large if H₀  |
|               | were true. Below 0.05 then it is *statistically  |
|               | significant*                                     |
+---------------+--------------------------------------------------+
| **η²**        | SS~between~ ÷ SS~total~: the proportion of all   |
|               | the variability in scores that is explained by   |
|               | which group you are in (an effect size; tick it  |
|               | under *Effect Size*)                             |
+---------------+--------------------------------------------------+
...
+---------------+---------------+---------------+---------------+
| Design today  | df            | t-table value | Tukey         |
|               |               |               | adjusted      |
|               |               |               | t-value       |
+===============+===============+===============+===============+
| Task 1:       | 42            | 2.02          | **2.43**      |
...
|                | Cohen's *d*      | η²                         |
+================+==================+============================+
| **What it is** | A *distance*,    | A *proportion*, how much   |
|                | how far apart    | of the variance the factor |
|                | two means are    | explains                   |
+----------------+------------------+----------------------------+
| **Units**      | Standard         | A fraction of 1 (read it   |
|                | deviations       | as a %)                    |
+----------------+------------------+----------------------------+
| **Applies to** | One pair of      | The whole ANOVA (all       |
|                | groups           | groups at once)            |
+----------------+------------------+----------------------------+
| **Small /      | 0.2 / 0.5 / 0.8  | 0.01 / 0.06 / 0.14         |
| medium /       |                  |                            |
| large**        |                  |                            |
+----------------+------------------+----------------------------+
| **Say it as**  | "the means are   | "recovery method explains  |
|                | 0.89 SD apart"   | 69% of the variability"    |
+----------------+------------------+----------------------------+
| **Never say**  | "explains 89% of | "the groups are 0.69 SD    |
|                | the variance"    | apart" or "69% chance they |
|                |                  | differ"                    |
+----------------+------------------+----------------------------+
...
-   Task 1:
    [**Recovery_HRV.csv**](https://lms.griffith.edu.au/courses/32192/files/9304106?wrap=1)
    --- 45 athletes, `Recovery Method` (Ice Bath / Compression / Active
    Recovery) and `RMSSD (ms)`
-   Task 2:
    [**Antihistamine_Delivery.csv**](https://lms.griffith.edu.au/courses/32192/files/9304108?wrap=1)
